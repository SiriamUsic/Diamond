...
async def play(c: Client, m: Message):
    ...
    if do.count("left") or do.count("Bad Request: user not found"):
        ...
        try:
            ...
            if b.status == "kicked":
                await m.reply_text(
                    f"@{ASSISTANT_NAME} **محظور في المجموعه** {m.chat.title}\n\n» **قم بفك حظر المستخدم اولا او تأكد من تقييد حساب المساعد**"
                )
                return
            ...
            if replied.audio or replied.voice:
                ...
                if chat_id in QUEUE:
                    ...
                    try:
                        ...
                        await m.reply_photo(
                            photo=f"{IMG_2}",
                            caption=f"-› اެݪحِاެݪةِ : تَمِ اެݪتَشِغِيَتم التشغيلِ\n🏷 -› الاسم : [{songname}]({link})\n💭-› ايدي المحادثه: {chat_id}`\n💡 ** الحالة:**تم التشغيلِ`\n🎧 **-› طلب الكائن دا:** {requester}\n📹 ** نوع البث:** `موسيقى",
                            reply_markup=keyboard,
                        )
                    except Exception as e:
                        await suhu.delete()
                        await m.reply_text(f"🚫 تأكد من وجود مكالمه مفتوحه اولا َآ:\n\n» {e}")
            else:
                if len(m.command) < 2:
                    await m.reply(
                        "رد على **ملف صوتي** او اكتب **اسم اغنيه** لتشغيلهاّ**"
                    )
                else:
                    suhu = await c.send_message(chat_id, "🔍**جاري البحثّ...**")
                    ...
                    if search == 0:
                        ...
                        if veez == 0:
                            ...
                            if chat_id in QUEUE:
                                ...
                                try:
                                    await suhu.edit("🔄 ** يتم انضمام المساعد للتشغيلَ...**")
                                    await call_py.join_group_call(
                                        chat_id,
                                        AudioPiped(
                                            ytlink,
                                    ...
                                    await suhu.delete()
                                    await m.reply_text(f"🚫 تأكد من وجود مكالمه مفتوحه اولا : `{ep}`")

        else:
            if len(m.command) < 2:
                await m.reply(
                    "⌯ رد على **ملف صوتي** او اكتب **اسم اغنيه** لتشغيلها.**"
                )
            else:
                    ...
                    if veez == 0:
                        ...
                        if chat_id in QUEUE:
                            ...
                            try:
                                ...
                                await suhu.delete()
                                await m.reply_text(f"تأكد منوجود مكالمه مفتوحه واعد المحاوله`{ep}`")
