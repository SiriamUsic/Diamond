...
@Client.on_message(command(["تشغيل","شغل","صلاح","/play","ش", f"play@{BOT_USERNAME}"]) & other_filters)
async def play(c: Client, m: Message):
    ...
    if do.count("left") or do.count("Bad Request: user not found"):
        ...
        try:
            ...
            if b.status == "kicked":
                await m.reply_text(
                    f"@{ASSISTANT_NAME} **ّمحظور في المجموعه** {m.chat.title}\n\n» **قم بفك حظر المستخدم اولا او تأكد من تقييد الحساب المساعد.**"
                )
                return
        ...
        if replied:
            if replied.audio or replied.voice:
                suhu = await replied.reply("**جاري البحث.. انتظر قليلا 👨🏻‍💻.**")
                ...
                if chat_id in QUEUE:
                    ...
                    await m.reply_photo(
                        photo=f"{IMG_1}",
                        caption=f"💡 ***-› تم اضافتها لقائمة الانتظار** `{pos}`\n\n🏷 **-› الاسم :** [{songname}]({link})| موسيقى`\n💭**-› ايدي المحادثه :** `{chat_id}`\n🎧 **-› طلب بوسطة :** {m.from_user.mention()}",
                    ...
                    try:
                        ...
                        await suhu.delete()
                        await m.reply_text(f"تأكد من وجود مكالمه مفتوحه وحاول مجدداً:\n\n» {e}")
            else:
                if len(m.command) < 2:
                    await m.reply(
                        "رد على ملف صوتي او اكتب شيئا للبحث عنه**"
                    )
                else:
                    ...
                    if search == 0:
                        await suhu.edit("❌ **لم يتم ور على نتائج**")
                    else:
                        songname = search[0]
                        url = search[1]
                        duration = search[2]
                        ...
                        if veez == 0:
                            ...
                            if chat_id in QUEUE:
                                ...
                                try:
                                    await suhu.edit("🔄 ** جاري انضمام المساعد للتشغيل...**")
                                    await call_py.join_group_call(
                                        chat_id,
                                        AudioPiped(
                                            ytlink,
                                    ...
                                    await suhu.delete()
                                    await m.reply_text(f"تأكد من وجود مكالمه مفتوحه وحاول مجدداً : `{ep}`")

        else:
            if len(m.command) < 2:
                await m.reply(
                    "رد على ملف صوتي او اكتب شيئا للبحث"
                )
            else:
                suhu = await c.send_message(chat_id, "🔍**جاري البحث...**")
                query = m.text.split(None, 1)[1]
                    ...
                    if veez == 0:
                        ...
                        if chat_id in QUEUE:
                            ...
                            try:
                                ...
                                await suhu.delete()
                                await m.reply_text(f"تأكد من وجود مكالمه مفتوحه حاول مجدداً`{ep}`")